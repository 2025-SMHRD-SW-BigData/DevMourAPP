...
class ReportActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun updatePhotoPreview() {
        ...
        selectedImages.forEachIndexed { index, bitmap ->
            ...
            // 이미지는 테두리보다 작게 만들어서 가운데 배치
            val dp120 = (120 * resources.displayMetrics.density).toInt()
            imageView.layoutParams = FrameLayout.LayoutParams(dp120, dp120).apply {
            ...
            deleteButton.layoutParams = FrameLayout.LayoutParams(dp24, dp24).apply {
...
<<<<<<< HEAD
        ...
        if (selectedImages.isEmpty()) {
            ...
            val guideText = TextView(this)
            guideText.text = "사진을 촬영하거나 앨범에서 선택해주세요"
            guideText.textSize = 16f
            guideText.setTextColor(ContextCompat.getColor(this, R.color.gray_text))
            val dp8 = (8 * resources.displayMetrics.density).toInt()
            guideText.layoutParams = FrameLayout.LayoutParams(
    ...
    private fun submitToServer(address: String, category: String, latitude: Double, longitude: Double) {
        ...
        // 상세 로그 시작
        Log.d("ReportActivity", "=== 민원 제출 시작 (직접 S3 업로드) ===")
        ...
        // 코루틴으로 S3 업로드 및 서버 제출 실행
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val s3Uploader = com.example.devmour.util.S3Uploader()
                val imageUrls = mutableListOf<String>()
                
                // 이미지가 있으면 S3에 업로드
                if (selectedImages.isNotEmpty()) {
                    Log.d("ReportActivity", "S3 이미지 업로드 시작...")
                    val uploadResult = s3Uploader.uploadMultipleBitmaps(selectedImages, "reports", this@ReportActivity)
                    
                    if (uploadResult.isSuccess) {
                        imageUrls.addAll(uploadResult.getOrThrow())
                        Log.d("ReportActivity", "S3 업로드 성공: ${imageUrls.size}개 이미지")
                        imageUrls.forEachIndexed { index, url ->
                            Log.d("ReportActivity", "이미지 ${index + 1} URL: $url")
                        }
                    } else {
                        Log.e("ReportActivity", "S3 업로드 실패: ${uploadResult.exceptionOrNull()?.message}")
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@ReportActivity, "이미지 업로드 실패: ${uploadResult.exceptionOrNull()?.message}", Toast.LENGTH_LONG).show()
                        }
                        return@launch
                    }
                }
                
                // 서버에 민원 데이터 제출 (이미지 URL 포함)
                Log.d("ReportActivity", "서버에 민원 데이터 제출 시작...")
                submitReportToServer(address, category, latitude, longitude, imageUrls)
                
            } catch (e: Exception) {
                Log.e("ReportActivity", "민원 제출 중 예외 발생", e)
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@ReportActivity, "민원 제출 중 오류가 발생했습니다: ${e.message}", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    
    private suspend fun submitReportToServer(
        address: String, 
        category: String, 
        latitude: Double, 
        longitude: Double, 
        ...
        try {
            ...
            // 이미지 URL을 파일명으로 변환하여 전송 (서버에서 URL로 인식하도록)
            var filePart1: MultipartBody.Part? = null
            if (imageUrls.isNotEmpty()) {
                val imageUrl = imageUrls[0]
                val fileName = imageUrl.substringAfterLast("/")
                val requestBody = imageUrl.toRequestBody("text/plain".toMediaType())
                filePart1 = MultipartBody.Part.createFormData("c_report_file1", fileName, requestBody)
                Log.d("ReportActivity", "첫 번째 이미지 URL 전송: $imageUrl")
            }
            ...
            withContext(Dispatchers.Main) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@ReportActivity, "서버 오류: ${response.code()}", Toast.LENGTH_LONG).show()
                }
            }
            
        } catch (e: Exception) {
            Log.e("ReportActivity", "서버 제출 중 오류", e)
            withContext(Dispatchers.Main) {
                Toast.makeText(this@ReportActivity, "서버 제출 중 오류: ${e.message}", Toast.LENGTH_LONG).show()
            }
        }
    }
